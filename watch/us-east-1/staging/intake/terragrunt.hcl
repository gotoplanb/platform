# Intake stack for watch / staging / us-east-1 (platform#8). Same as prod. NOTE: the VPC
# consumer Lambda needs NAT (Secrets Manager) — lean staging has none; applying staging
# intake needs enable_nat or Secrets Manager/STS VPC endpoints. The ingest path (API GW ->
# SQS + authorizer) works regardless.
...
dependency "config" {
  ...
  mock_outputs = {
    ...
    intake_webhook_secret_param_name = "/watch/staging/intake-webhook-secret"
