# Escalation stack for watch / prod / us-east-1 (platform#7). Step Functions Standard
# state machine + decision Lambdas (VPC, RDS) + failed-execution alarm. The state machine
# name == "${project}-${env}", matching the ARN the app stack predicts (#6). Needs NAT for
# the Lambdas to reach Secrets Manager — prod is ha (NAT).
