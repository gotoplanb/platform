# Status-page DNS record for watch / prod / us-east-1 (ADR-020). status.<domain> CNAME -> the
# frontend CloudFront distribution. Split from ../dns (the API record) so the CloudFront
# new-account verification hold can't block watch.<domain>. This stack stays blocked until
# prod/frontend applies (i.e. until AWS lifts the CloudFront hold); the API stays reachable
# regardless. Only the status. record here — never the apex. Needs CLOUDFLARE_API_TOKEN.
